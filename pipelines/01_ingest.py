...
def run():
    ...
    if not os.path.exists(RAW_DATA_PATH):
        ...
        print("To generate synthetic mock data for testing, run: python tests/data_quality/mock_data.py")
        sys.exit(1)
    ...
    try:
        ...
        if not required_cols.issubset(set(df.columns)):
            raise ValueError(f"Data is missing required columns. Needs: {required_cols}")

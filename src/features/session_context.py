...
def add_session_context_features(df: pl.DataFrame) -> pl.DataFrame:
    out = df.sort("timestamp").with_columns(pl.col("timestamp").dt.date().alias("_date"))
    daily = (
        out.group_by("_date")
        .agg(
        ...
        .with_columns(
            ...
            (pl.col("_prev_day_high") - pl.col("_prev_day_low")).alias("_prev_day_range"),
        )
    )
    return (
        out.join(daily, on="_date", how="left")
        .with_columns(
            (pl.col("open") - pl.col("_prev_day_close")).alias("f_gap_from_prev_close"),
            (
                (pl.col("open") - pl.col("_prev_day_close")) / (pl.col("_prev_day_close") + 1e-9)
            ).alias("f_gap_from_prev_close_pct"),
            (pl.col("close") - pl.col("_prev_day_high")).alias("f_distance_prev_high"),
            (pl.col("close") - pl.col("_prev_day_low")).alias("f_distance_prev_low"),
            (
            ...
            (pl.col("high").cum_max().over("_date") - pl.col("close")).alias(
                "f_intraday_high_pullback"
            ),
            (pl.col("close") - pl.col("low").cum_min().over("_date")).alias(

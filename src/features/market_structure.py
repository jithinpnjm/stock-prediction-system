...
def add_market_structure_features(df: pl.DataFrame) -> pl.DataFrame:
    ...
    out = out.with_columns(
        pl.Series("_last_swing_high", last_high),
        pl.Series("_last_swing_low", last_low),
        ...
        (
            (pl.col("close") - pl.col("_last_swing_high"))
            / (pl.col("_last_swing_high") - pl.col("_last_swing_low") + eps)
        ).alias("f_position_vs_structure_high"),
        (
            (pl.col("close") - pl.col("_last_swing_low"))
            / (pl.col("_last_swing_high") - pl.col("_last_swing_low") + eps)
        ).alias("f_position_vs_structure_low"),
        (pl.col("close") - pl.col("_last_swing_high")).alias("f_distance_to_last_swing_high"),
        (pl.col("close") - pl.col("_last_swing_low")).alias("f_distance_to_last_swing_low"),
        (pl.col("close") > pl.col("_last_swing_high"))
        .cast(pl.Int8)
        ...
        (pl.col("close").rolling_mean(12) - pl.col("close").rolling_mean(48)).alias(
            "f_trend_context"
        ),
        (pl.col("high").rolling_max(12) - pl.col("low").rolling_min(12)).alias(
            "f_short_structure_range"
        ),
    )
    return out.drop(["_last_swing_high", "_last_swing_low"])

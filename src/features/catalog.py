...
FEATURE_CONTRACTS = (
    ...
    FeatureContract("opening_range_", "5m", "completed/partial causal range", "5m close"),
    FeatureContract("position_in_opening_", "5m", "completed/partial causal range", "5m close"),
...
def unknown_feature_columns(df: pl.DataFrame) -> list[str]:
    ...
    for column in df.columns:
        if column in ignored:
            continue
        if not any(column.startswith(c.name_prefix) for c in FEATURE_CONTRACTS):

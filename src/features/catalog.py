...
FEATURE_CONTRACTS = (
    ...
    FeatureContract("range_to_atr_", "5m", "rolling current bars", "5m close"),
    ...
    FeatureContract("consecutive_length", "5m", "causal cluster", "5m close"),
    FeatureContract("cluster_", "5m", "causal cluster", "5m close"),
    FeatureContract("has_cluster_", "5m", "causal cluster", "5m close"),
    ...
    FeatureContract("bos_", "5m", "past/current structure", "5m close"),
    ...
    FeatureContract("minute_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("path_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("intrabar_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("up_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("down_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("first_last_", "1m", "completed 5m bucket", "5m close"),
    FeatureContract("cusum_", "5m", "past/current returns", "5m close"),
)


def assert_feature_frame_is_pre_label(
    df: pl.DataFrame,
) -> None:
    forbidden = {
        "label",
...
def unknown_feature_columns(
    ...
    return sorted(
        ...
        and not any(
            column.startswith(contract.name_prefix)
            for contract in FEATURE_CONTRACTS
        )
    )


def assert_known_feature_columns(
    df: pl.DataFrame,
) -> None:
    unknown = unknown_feature_columns(df)
    if unknown:
        raise ValueError(
            "Unregistered feature columns detected: "
            + ", ".join(unknown)
        )

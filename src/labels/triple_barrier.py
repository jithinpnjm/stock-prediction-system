...
def _label_path(close,high,low,start_idx,end_idx,target_pts,stop_pts):
    ...
    for i in range(n):
        ...
        for j in range(start_idx[i],end_idx[i]):
            ...
            ml=max(ml,up); al=max(al,down); ms=max(ms,down); ass=max(ass,up)
            if lti==-1 and lsi==-1:
                if high[j]>=lt and low[j]<=ls: lsi=j
                elif high[j]>=lt: lti=j
                elif low[j]<=ls: lsi=j
            if sti==-1 and ssi==-1:
                if low[j]<=st and high[j]>=ss: ssi=j
                elif low[j]<=st: sti=j
        ...
        if long_valid and short_valid:
            if lti<=sti: labels[i]=1; hit_idx[i]=lti
            else: labels[i]=-1; hit_idx[i]=sti
        elif long_valid: labels[i]=1; hit_idx[i]=lti
        elif short_valid: labels[i]=-1; hit_idx[i]=sti
...
def apply_triple_barrier_labels(
    ...
    *,
    ...
    dates_e=events["timestamp"].dt.date().to_list()
    dates_b=bars["timestamp"].dt.date().to_list()
    ...
    for i,ts in enumerate(e_ts):
        j=int(starts[i]); end=int(np.searchsorted(b_ts,ts+horizon_ns,side="right"))
        while end>j and dates_b[end-1]!=dates_e[i]: end-=1
        ends[i]=end
    ...
    starts=starts[usable]; ends=ends[usable]; e_ts=e_ts[usable]
    ...
    expiry=pl.Series("event_end_timestamp",b_ts[expiry_idx]).cast(
        pl.Datetime(time_zone="Asia/Kolkata")
    )
    barrier=pl.Series("barrier_timestamp",b_ts[barrier_idx]).cast(
        pl.Datetime(time_zone="Asia/Kolkata")
    )
    result=events.with_columns(
        pl.Series("label",labels,dtype=pl.Int8),
        expiry,barrier,
        pl.Series("mfe_long_points",mfe_l),pl.Series("mae_long_points",mae_l),
        pl.Series("mfe_short_points",mfe_s),pl.Series("mae_short_points",mae_s),

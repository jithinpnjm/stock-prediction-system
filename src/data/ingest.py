...
def _normalize_timestamp(series: pl.Series, input_timezone: str | None) -> pl.Series:
    ...
    if series.dtype.time_zone is None:
        if not input_timezone:
            raise ValueError(
                "Naive timestamps require an explicit input_timezone; refusing "
                "to guess timestamp semantics."
            )
...
def normalize_ohlcv(
    df: pl.DataFrame,
    *,
    input_timezone: str | None = MARKET_TIMEZONE,
    ...
    df = df.with_columns(
        _normalize_timestamp(df.get_column("timestamp"), input_timezone).alias("timestamp")
    )

    numeric = [
        pl.col(c).cast(pl.Float64, strict=True).alias(c)
        for c in ("open", "high", "low", "close", "volume")
    ]
    df = df.with_columns(numeric)
    df = df.with_columns(
        pl.col("timestamp").dt.date().alias("session_date")
    ).select(
        ["session_date", "timestamp", "open", "high", "low", "close", "volume"]
    )

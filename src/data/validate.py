...
def validate_ohlcv(
    ...
    *,
    ...
    reports = [_report("non_empty", True, count=df.height)]
    ...
    if session.is_empty():
        ...
        reports.append(
            _report("unexpected_intraday_spacing", False, 0, "no in-session rows")
        )
    ...
    reports.append(
        _report(
            "unexpected_intraday_spacing",
            cadence_errors == 0,
            cadence_errors,
            "all consecutive in-session timestamps must be exactly 60 seconds apart",
        )
    )
...
def assert_valid(
    ...
    *,
    ...
    critical = [
        ...
        if report["gate"] != "stale_candle_diagnostic" and not report["passed"]
    ]
    if critical:
        raise ValueError(
            "Data quality validation failed: "
            + "; ".join(
                f"{report['gate']}({report['count']})"

...
class ExecutionHandler:
    def __init__(self, portfolio: Portfolio, slippage: float = 2.0, commission: float = 20.0):
    ...
    def execute_trade(self, time: str, price: float, signal: int, size: float = 15.0):
        ...
        if self.portfolio.position != 0 and (signal != self.portfolio.position or signal == 0):
    ...
    def _close_position(self, time: str, price: float):
        ...
        self.portfolio.cash += (self.portfolio.entry_price * self.portfolio.position_size * direction) # free up margin (mocked)
